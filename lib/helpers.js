...
helpers.createRandomString = function (strLength) {
    ...
    if (strLength) {
        // Define all the possible characters that could go intro a string
    ...
    if (templateName) {
        ...
        fs.readFile(templatesDir + templateName + '.html', 'utf8', function (err, str) {
            if (!err && str && str.length > 0) {
                // Do interpollation on the st4ring
                var finalString = helpers.interpolate(str, data);
                callback(false, finalString);
            } else {
                callback('No template caould be found');
            }
        });
    } else {
...
// Add the universal header and footr to a string, and pass provided to the header and footer for interpollation
